...
export const CheckCostGuardOutputSchema = z.object({
  verdict: z.enum(['PASS', 'AMBER', 'ZERO']),
  line_findings: z.array(
    z.object({
      lineNo: z.string(),
      qty_x_rate: z.number(),
      draftAmount: z.number(),
...
export type CheckCostGuardOutput = z.infer<typeof CheckCostGuardOutputSchema>;

const AMOUNT_EPSILON = 0.01;
const VARIANCE_THRESHOLD = 2;

export async function run(input: CheckCostGuardInput): Promise<CheckCostGuardOutput> {
  const line_findings = input.lines.map((line) => {
    ...
    if (Math.abs(qty_x_rate - line.draftAmount) > AMOUNT_EPSILON) {
      return {
        lineNo: line.lineNo,
        qty_x_rate,
        draftAmount: line.draftAmount,
        standardAmount: line.standardAmount,
        variance_pct: null,
        reason_code: 'QTY_X_RATE_MISMATCH'
      };
    }
    ...
    if (line.standardAmount !== null && line.standardAmount > 0) {
      ...
      if (Math.abs(std_variance_pct) > VARIANCE_THRESHOLD) {
        return {
          ...
          draftAmount: line.draftAmount,
          standardAmount: line.standardAmount,
          variance_pct: Math.round(std_variance_pct * 100) / 100,
    ...
    if (line.standardAmount === null) {
      return {
        ...
        standardAmount: line.standardAmount,
        variance_pct: null,
        reason_code: 'STANDARD_RATE_NOT_AVAILABLE'
      };
    }
  ...
  for (const finding of line_findings) {
    if (finding.reason_code === 'QTY_X_RATE_MISMATCH') {
      verdict = 'ZERO';
      break;
    }
    if (
      finding.reason_code === 'COST_VARIANCE_EXCEEDS_2PCT' ||
      finding.reason_code === 'STANDARD_RATE_NOT_AVAILABLE'
    ) {
      verdict = 'AMBER';
    }
  }
