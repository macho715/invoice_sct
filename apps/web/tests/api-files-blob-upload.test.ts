...
import { POST } from '../src/app/api/files/blob-upload/route';

beforeEach(() => handleUploadMock.mockReset());

describe('POST /api/files/blob-upload (client token minter)', () => {
  it('returns the handleUpload token payload', async () => {
    ...
    const r = await POST(new Request('http://test/api/files/blob-upload', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ type: 'blob.generate-client-token', payload: { pathname: 'inv.pdf', callbackUrl: 'http://test/api/files/blob-upload' } }),
    }));
