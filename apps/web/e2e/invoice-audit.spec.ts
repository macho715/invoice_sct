...
test.beforeEach(async ({ page, request }, testInfo) => {
  ...
  await page.context().route('**/api/**', async (route) => {
    // Allow the real request to pass through; this hook just sets a default header.
    const req = route.request();
    const headers = { ...req.headers(), 'x-user-id': 'e2e-user' };
...
test('1. uploads small PDF (< 4.5 MB) via /api/files/ingest', async ({ page, request }) => {
  ...
  const r = await request.post('/api/files/ingest', { multipart: { file: { name: 'e2e_small.pdf', mimeType: 'application/pdf', buffer: smallPdfBuffer() } } });
...
test('2. large PDF (>= 4.5 MB) requires client direct upload to Vercel Blob', async ({ request }) => {
  ...
  const direct = await request.get('/api/files/direct-upload', { failOnStatusCode: false });
...
test('3. polls /api/audit/status until terminal state', async ({ request, page }) => {
  ...
  while (Date.now() - start < 5_000) {
    const r = await request.get(`/api/audit/status?job_id=${job_id}`);
...
test('4. PASS verdict exposes signed URL with private access', async ({ request }) => {
  ...
  const r = await request.get(`/api/audit/result?job_id=${jobId}`);
  if (r.status() === 404) {
  ...
  const exp = await request.post('/api/audit/export', { data: { job_id: jobId, kind: 'FINAL_APPROVED' } });
...
test('5. AMBER verdict with variance < 500 AED requires Ops Lead', async ({ request }) => {
  ...
  const appr = await request.post('/api/audit/approve', {
    headers: { 'x-user-role': 'COST_CONTROL_LEAD', 'x-user-id': 'e2e-ops-lead' },
  ...
  const denied = await request.post('/api/audit/approve', {
    headers: { 'x-user-role': 'WAREHOUSE_MANAGER', 'x-user-id': 'e2e-other' },
...
test('6. AMBER verdict with variance >= 500 AED requires Finance Manager', async ({ request }) => {
  ...
  const denied = await request.post('/api/audit/approve', {
    headers: { 'x-user-role': 'COST_CONTROL_LEAD', 'x-user-id': 'e2e-ops' },
  ...
  const ok = await request.post('/api/audit/approve', {
    headers: { 'x-user-role': 'FINANCE_APPROVER', 'x-user-id': 'e2e-finance' },
...
// ---------------------------------------------------------------------------
// 7) ZERO verdict → Review Pack only (no FINAL_APPROVED export)
// ---------------------------------------------------------------------------

test('7. ZERO verdict blocks FINAL_APPROVED export — Review Pack only', async ({ request }) => {
  const jobId = newJobId();
  ...
  // 7a. FINAL_APPROVED export is NOT allowed.
  const blocked = await request.post('/api/audit/export', {
  ...
  expect(['EXPORT_BLOCKED', 'POLICY_VIOLATION', 'ZERO_NOT_EXPORTABLE', 'ZERO_BLOCKED']).toContain(blockedBody.code ?? '');
...
test('8. DLP blocks upload whose filename embeds a P2 pattern', async ({ request, page }) => {
  ...
  if (r.status() === 201) {
    ...
    const trace = await request.get(`/api/audit/trace?job_id=${body.job_id}`);
    if (trace.status() === 200) {
