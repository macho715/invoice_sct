...
> **상태**: 확정 (Final)
> **버전**: v1.00
> **이전 문서**: 2026-06-09-invoice-audit-phase2-plan.md, 2026-06-09-invoice-audit-phase3-plan.md
...
기존 문서의 핵심 흐름은 **Upload → Dry-run Audit → AMBER/ZERO → Excel Audit Pack Export**이며, invoice 검증, contract rate 비교, evidence mapping, TYPE-B matrix, final subtotal reconciliation, 7-sheet Excel Audit Pack export가 중심이다. 이 구조는 유지하되, 이번 정정에 따라 **SCT_ONTOLOGY는 파일 수집·PDF/OCR·Excel 생성까지 담당하는 통합 MCP가 아니라 판단 엔진으로 한정**한다.

### 1.3 구현 가능성 근거

Next.js Route Handler는 `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, `OPTIONS` 같은 HTTP method handler를 route file에서 만들 수 있으므로 `/api/files/upload`, `/api/invoice-audit/run`, `/api/audit/status`, `/api/export/download` 같은 Vercel API Orchestrator 구현이 가능하다.

Vercel Blob은 private/public access를 명시해 파일을 저장할 수 있고, `request.formData()`로 받은 파일을 `put()`으로 private Blob에 저장하는 예제를 제공한다. 다만 Vercel Function의 request/response payload 한도는 **4.5MB**이므로 대용량 invoice/evidence 파일은 Function body에 직접 싣지 않고 Blob upload 구조로 처리해야 한다.

Vercel Python Runtime은 Python Function과 FastAPI dependency 구성을 지원하지만, OCR·장시간 parsing·대형 workbook generation은 Function duration, memory, bundle size 제약을 받을 수 있으므로 Python Worker로 분리한다.

FastAPI는 `UploadFile` 기반 multipart file upload와 `FileResponse` 기반 file streaming response를 지원하므로 Python Worker에서 parser input과 xlsx export output을 처리할 수 있다.
...
| G8 | 7-sheet Audit Pack 생성       | 승인 후 xlsx 다운로드 가능                                                                                             |
...
| MVP Output               | JSON result, 7-sheet Audit Pack `.xlsx`                                                                              |
...
Vercel environment variables는 deployment 기준 전체 64KB 제한이 있으므로 contract rate JSON, ontology dump, audit rule dump 같은 대형 데이터는 environment variable에 넣지 않는다.
...
- 가정: PDF text extraction은 MVP 이후 Phase 3로 포함하고, OCR 이미지 PDF는 별도 OCR 서비스 선정 후 Phase 4에서 처리한다.
- 가정: Google Drive ingest는 OAuth/권한 설계 완료 전까지 MVP 범위에 넣지 않는다.
- 가정: xlsx export는 기존 7-sheet Audit Pack 기준을 따른다.
...
### Phase 2. 7-Sheet XLSX Export

| 항목            | 내용                                                          |
| ------------- | ----------------------------------------------------------- |
| 목적            | SCT validation 결과와 audit trace를 7-sheet Audit Pack으로 export |
| 구현 가능성        | **가능 / PASS**                                               |
...
| Task ID | Task                     | 완료 기준                               |
| ------- | ------------------------ | ----------------------------------- |
| P2-T1   | Export schema 확정         | 7개 sheet column 확정                  |
...
> **구현 현황**: `apps/worker-py/app/parsers/pdf_text.py` (P3A pdfplumber) 이미 구현됨. Phase 3 plan: `2026-06-09-invoice-audit-phase3-plan.md` 참조.
...
```mermaid
graph TD
    U[Vercel UI] -->|파일 선택| API1[POST /api/files/ingest]
    API1 -->|원본 저장| BLOB[(Private Blob)]
    API1 -->|job 생성| DB[(DB / Queue)]
    DB -->|queued| W[Python Worker]
    W -->|Blob 읽기| BLOB
    W -->|parse| PARSED[Parsed JSON]
    PARSED -->|SCT validate| SCT[SCT_ONTOLOGY MCP]
    SCT -->|gate result| GR[PASS / AMBER / ZERO]
    GR -->|저장| DB
    GR -->|dry-run 결과 표시| U
    U -->|승인| API2[POST /api/audit/approve]
    API2 -->|xlsx 생성 요청| W
    W -->|7-sheet Audit Pack| XLSX[.xlsx Export]
    XLSX -->|download| U
```

### 6.2 System Flow Tasks
...
|   10 | Python Worker            | xlsx 생성           | 7-sheet Audit Pack  |
...
| `build_audit_pack_xlsx` | 7-sheet workbook 생성              | `.xlsx`           |
| `write_jsonl_log`       | audit trace append               | JSONL log         |

### 6.5 SCT_ONTOLOGY Tasks
...
> ⚠️ 확인 필요: 현재 문서에는 "9개 MCP 함수 discover 완료"로 되어 있으나, 함수명·parameter·return schema는 final implementation 전에 별도 schema freeze가 필요하다.
...
|  7 | XLSX export             | 7-sheet Audit Pack 생성                                     |
...
| D8. XLSX Export                  | 7-sheet Audit Pack                                                | 승인 후 다운로드 가능                          |
...
*Last updated: 2026-06-10 | Owner: SCM Team | Version: v1.00*
