...
- 🔜 **예정**: ① OCR 후 variance 재계산 + `/api/audit/export` 1-click 트리거(검수→export 무결성 마감). ② 워커 `pdf_text.py`의 text_span → `invoice_lines` 실추출 (Phase 2.5) — PDF 단독을 AMBER가 아닌 실검증으로 승격. ③ `06_Rate_Check`/`04_Line_View`에 rate_match 신규 컬럼 노출 — 워커 익스포터(`xlsx.py`)에 컬럼 추가 필요.
...
| `apps/web/src/app/api/` | 13 API route handlers (incl. `/api/audit/vision-status` polling) |
...
- apps/web: 352 tests (44 files), `pnpm test` (verified 2026-06-17; incl. approval-gated Vision OCR tests)
