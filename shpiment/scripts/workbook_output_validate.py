...
REQUIRED_COLUMNS = {
    '00_Decision': ['Overall Verdict'],
    '01_Action_Items': ['Action_ID', 'Severity', 'Issue', 'Status'],
    '02_Final_Recon': ['Verdict'],
    '03_Type_B_Summary': ['Shipment_No', 'Customs', 'DO', 'INLAND', 'THC', 'Inspection', 'Detention', 'STROAGE', 'OTHERS', 'Total_AED', 'Total_USD', 'Line_Count'],
    '04_Line_View': ['Shipment_No', 'Source_Row_ID', 'Rate_Source', 'Description', 'Total_AED', 'Total_USD', 'TYPE_B', 'Evidence_Status'],
    '90_Source_Data': ['Source_Row_ID'],
    '91_Audit_Detail': ['Source_Row_ID', 'Shipment_No', 'TYPE_B', 'Evidence_Status', 'Amount_Check', 'Risk'],
    '92_Evidence_Issues': ['Issue_ID', 'TYPE_B', 'Evidence_Status', 'Severity', 'Action_Required'],
}
...
def validate_workbook(path: Path, require_final_subtotal=False, dlp_mode='internal', tolerance=0.01):
    ...
    if source_rows and line_rows and len(source_rows) != len(line_rows):
        errors.append(f'source_line_count != line_audit_count: {len(source_rows)} vs {len(line_rows)}')

    line_hmap, _ = header_map(sheet_data.get('04_Line_View', []))
    typeb_idx = get_col_idx(line_hmap, ['TYPE_B'])
    ev_idx = get_col_idx(line_hmap, ['Evidence_Status'])
    line_total_aed_idx = get_col_idx(line_hmap, ['Total_AED', 'TOTAL AMOUNT AED'])
    line_total_usd_idx = get_col_idx(line_hmap, ['Total_USD', 'TOTAL AMOUNT USD'])
    ...
    if line_rows:
        if typeb_classified_count != len(line_rows):
            errors.append(f'type_b_classified_count != line_audit_count: {typeb_classified_count} vs {len(line_rows)}')
        if evidence_status_count != len(line_rows):
            errors.append(f'evidence_status_count != line_audit_count: {evidence_status_count} vs {len(line_rows)}')
    ...
    if line_total_usd is not None and typeb_total_usd is not None:
        ...
        if abs(delta) > tolerance:
            errors.append(f'line vs TYPE-B USD delta exceeds tolerance: {delta}')
